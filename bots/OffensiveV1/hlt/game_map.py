...
class GameMap:
    ...
    def navigate(self, source, target, offset=1, ignore_dropoff=False):
        direction = self.dijkstra_a_to_b(source, target, offset=offset)
        new_position = source.directional_offset(direction)
        logging.debug(f"source: {source} and target: {target} and new position: {new_position}")
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if cell.has_structure and cell.structure.owner != self.me:
                    self.enemy_bases.append(Position(x, y))
